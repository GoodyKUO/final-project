...
int play(char Chessboard[][7], int ln, int x, 
	int y, char ChessPieces)
{
...
//判断是否連成五子且一樣//Chessboard=棋盤,ln=棋盤寬度,(XS,YS)方向判斷起點座標,(dx,dy)方向增量
//有成功return 1，没有return 0
int judge_line(char Chessboard[][7], int ln, int XS, int YS, int dx, int dy)
{
	if ((XS <ln) && (YS<ln)             //起点點座標在棋盤內
		&& (XS >= 0) && (YS >= 0)
		&& (dx != 0 || dy != 0))        //座標增量不同時為0
	{
		if (((XS + dx * 4) > ln) || ((XS + dx * 4)<0) || //判斷终點座標
			((YS + dy * 4)>ln) || ((YS + dy * 4) < 0) || //在棋盤外
			('\t' == Chessboard[XS][YS]))
		{
			return 0;  //不在棋盤内or起點没下棋子
		}
		else
		{
			for (int i = 1; i < 5; ++i){
